...
var VirtualList = React.createClass({
    ...
    getVirtualState_: function getVirtualState_(props) {
        this.getVirtualState(props, false);
    },
    getVirtualState: function getVirtualState(props) {
        var mounted = arguments.length <= 1 || arguments[1] === undefined ? true : arguments[1];
        ...
        if (typeof props.container === 'undefined' || props.items.length === 0 || props.itemHeight <= 0 || !mounted) return state;

        var items = props.items;

        if (mounted) {
            var container = ReactDOM.findDOMNode(props.container);
        ...
        viewBox.top = utils.viewTop(ReactDOM.findDOMNode(props.container));
    ...
    getInitialState: function getInitialState() {
        return this.getVirtualState(this.props, false);
    },
    ...
    _getViewBox: function _getViewBox(nextProps) {
        var container = ReactDOM.findDOMNode(nextProps.container);
        return {
            height: typeof container.innerHeight !== 'undefined' ? container.innerHeight : container.clientHeight
        };
    },
    _getListBox: function _getListBox(nextProps) {
        //var list = this.getDOMNode();
        var list = ReactDOM.findDOMNode(this);

        var top = utils.topDifference(list, ReactDOM.findDOMNode(nextProps.container));
    ...
    componentWillReceiveProps: function componentWillReceiveProps(nextProps) {
        ...
        var state = this.getVirtualState(nextProps, this.isMounted());

        this.state.container.removeEventListener('scroll', this.onScrollDebounced);

        this.onScrollDebounced = utils.debounce(this.onScroll, nextProps.scrollDelay, false);

        ReactDOM.findDOMNode(nextProps.container).addEventListener('scroll', this.onScrollDebounced);
    ...
    componentDidMount: function componentDidMount() {
        ...
        ReactDOM.findDOMNode(this.props.container).addEventListener('scroll', this.onScrollDebounced);
    },
